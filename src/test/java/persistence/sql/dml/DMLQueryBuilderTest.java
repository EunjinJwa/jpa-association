...
public class DMLQueryBuilderTest {
    ...
    public void selectJoinQueryTest() {
        ...
        String query = dmlQueryBuilder.selectJoinQuery(entityMetadata, joinEntityMeta, 1L);

        assertEquals("SELECT order_items.id, order_items.product, order_items.quantity FROM orders LEFT JOIN order_items ON orders.id = order_items.order_id WHERE orders.id = 1", query);
    }
